...
def get_events():
    ...
    list_test = db.session.query(EventsModel).join(EventsUsersModel,
                EventsModel.id == EventsUsersModel.event_id).filter(
                or_(EventsModel.user_id == 6, EventsUsersModel.member_id == 6)).group_by(EventsModel.id).all()
